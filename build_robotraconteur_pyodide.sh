...
SIDE_C_FLAGS="-s DISABLE_EXCEPTION_CATCHING=0 -fpic"
SIDE_LDFLAGS="-s \"BINARYEN_METHOD='native-wasm'\" -Werror -s EMULATED_FUNCTION_POINTERS=1 -s EMULATE_FUNCTION_POINTER_CASTS=1 -s SIDE_MODULE=1 -s WASM=1 -s \"BINARYEN_TRAP_MODE='clamp'\" --memory-init-file 0 -s LINKABLE=1 -s EXPORT_ALL=1 -s DISABLE_EXCEPTION_CATCHING=0"
...
# Also add robotraconteur to packages.json in $PYODIDE_ROOT/build
