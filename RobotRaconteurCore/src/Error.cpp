...
namespace RobotRaconteur
{
	...
	RobotRaconteurException::RobotRaconteurException(MessageErrorType ErrorCode, const std::string &error, const std::string &message, std::string sub_name, RR_INTRUSIVE_PTR<RRValue> param_) : std::runtime_error((error + " " + message).c_str())
	{
		InitializeInstanceFields();
		Error = error;
		Message=message;
...
#define RR_EXCEPTION_DEF_1(exp_cpp_type, exp_code, exp_type_str) \
	exp_cpp_type::exp_cpp_type(const std::string &message, std::string sub_type, RR_INTRUSIVE_PTR<RRValue> param_) : RobotRaconteurException(exp_code,exp_type_str,message,sub_type,param_) \
	{ \
	} \

#define RR_EXCEPTION_DEF_2(exp_cpp_type, exp_code) \
	exp_cpp_type::exp_cpp_type(const std::string &error, const std::string &message, std::string sub_type, RR_INTRUSIVE_PTR<RRValue> param_) : RobotRaconteurException(exp_code, error, message, sub_type, param_) \
	{ \
	} \
	exp_cpp_type::exp_cpp_type(std::exception &innerexception) : RobotRaconteurException(exp_code, typeid(innerexception).name(), innerexception.what(), "", RR_INTRUSIVE_PTR<RRValue>()) \
	{ \
	} \
...
#define RR_DownCastException_M2(exp_cpp_type, exp_code) \
	...
	RR_SHARED_PTR<RobotRaconteurException> RobotRaconteurExceptionUtil::DownCastException(RobotRaconteurException& err)
	{
		...
		return RR_MAKE_SHARED<RobotRaconteurException>(err.ErrorCode,err.Error,err.Message, err.ErrorSubName,err.ErrorParam);
	...
	RR_SHARED_PTR<RobotRaconteurException> RobotRaconteurExceptionUtil::ExceptionToSharedPtr(std::exception& err, MessageErrorType default_type)
	{
		...
		else
		{
			...
			{
				RobotRaconteurException err3(default_type, std::string(typeid(err).name()), std::string(err.what()), "", RR_INTRUSIVE_PTR<RRValue>());
