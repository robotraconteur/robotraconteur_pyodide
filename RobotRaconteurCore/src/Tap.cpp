...
namespace RobotRaconteur
{
    namespace detail
    {
        ...
        {
        public:
            Tap_acceptor(RR_BOOST_ASIO_IO_CONTEXT& context, boost::asio::local::stream_protocol::endpoint ep ) : acceptor(context, ep) {}
    ...
    namespace detail
    {
        ...
        public:
            ...
            void end_send(const boost::system::error_code& ec, std::size_t bytes_transferred)
            {
                ...
                {
                    socket->socket->async_write_some(boost::asio::buffer(send_buffer.get() + message_pos, message_len - message_pos),
                    boost::bind(&LocalMessageTapConnectionImpl::end_send,shared_from_this(),
                        boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));
                    return;
                }
        ...
        public:
            ...
            void Open(const std::string& tap_name)
            {
                ...
                try
                {
	                ...
	                all_acceptor1->acceptor.async_accept(*all_socket1->socket,boost::bind(&LocalMessageTapImpl::handle_accept_all,shared_from_this(),all_acceptor1,all_socket1,boost::asio::placeholders::error));

                    RR_SHARED_PTR<detail::Tap_socket> log_socket1(new detail::Tap_socket(*io_context));
	                log_acceptor1->acceptor.async_accept(*log_socket1->socket,boost::bind(&LocalMessageTapImpl::handle_accept_log,shared_from_this(),log_acceptor1,log_socket1,boost::asio::placeholders::error));

                    boost::thread thread(boost::bind(&LocalMessageTapImpl::thread_func,shared_from_this(),io_context,all_tap_fname,log_tap_fname));
            ...
            void handle_accept_all(RR_SHARED_PTR<detail::Tap_acceptor> acceptor, RR_SHARED_PTR<detail::Tap_socket> socket, const boost::system::error_code& error)
            {
                ...
                RR_SHARED_PTR<detail::Tap_socket> all_socket1(new detail::Tap_socket(*io_context));
                acceptor->acceptor.async_accept(*all_socket1->socket,boost::bind(&LocalMessageTapImpl::handle_accept_all,shared_from_this(),acceptor,all_socket1,boost::asio::placeholders::error));

            }

            void handle_accept_log(RR_SHARED_PTR<detail::Tap_acceptor> acceptor, RR_SHARED_PTR<detail::Tap_socket> socket, const boost::system::error_code& error)
            {
                if (error)
                    return;
                if (!is_open)
                ...
                RR_SHARED_PTR<detail::Tap_socket> log_socket1(new detail::Tap_socket(*io_context));
                acceptor->acceptor.async_accept(*log_socket1->socket,boost::bind(&LocalMessageTapImpl::handle_accept_log,shared_from_this(),acceptor,log_socket1,boost::asio::placeholders::error));
            }

            static void thread_func(RR_SHARED_PTR<LocalMessageTapImpl> this1,  RR_SHARED_PTR<RR_BOOST_ASIO_IO_CONTEXT> io_context1, 
                boost::filesystem::path all_tap_fname1, boost::filesystem::path log_tap_fname1)
            {
    ...
    LocalMessageTap::LocalMessageTap(const std::string& tap_name)
    {
