...
namespace RobotRaconteur
{
	...
	Transport::Transport(RR_SHARED_PTR<RobotRaconteurNode> node)
	{
		this->node = node;
	}
	...
	void Transport::FireTransportEventListener(RR_SHARED_PTR<Transport> shared_this,TransportListenerEventType ev, RR_SHARED_PTR<void> parameter)
	{
	...
	void Transport::AsyncGetDetectedNodes(const std::vector<std::string>& schemes, boost::function<void(RR_SHARED_PTR<std::vector<NodeDiscoveryInfo> >)>& handler, int32_t timeout)
	{
	...
	ROBOTRACONTEUR_CORE_API ParseConnectionURLResult ParseConnectionURL(const std::string& url)
	{
		...
		if (o.scheme == "") throw ConnectionException("Invalid Connection URL");
		
		if (o.scheme == "tcp")
		{
			if (url_result.size() < 5) throw ConnectionException("Invalid Connection URL");

			o.host = url_result[2];
			if (o.host == "") throw ConnectionException("Invalid Connection URL");
			...
			if (s.size() < 2) throw ConnectionException("Invalid Connection URL");
		...
		if (url_result.size() < 6) throw ConnectionException("Invalid Connection URL");
		
		if (boost::starts_with(url, o.scheme + ":////")) throw ConnectionException("Invalid Connection URL");
		...
		BOOST_FOREACH (std::string& e, q2)
		{
			...
			if (q3.size() != 2) throw ConnectionException("Invalid Connection URL");
