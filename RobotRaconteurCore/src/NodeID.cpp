...
{
	if (id.size()!=16) throw InvalidArgumentException("Invalid NodeID");
	std::copy(id.begin(),id.end(),this->id.begin());		
}

NodeID::NodeID(const NodeID& id)
{
	if (id.id.size()!=16) throw InvalidArgumentException("Invalid NodeID");
	this->id=id.id;
}
...
NodeID::NodeID(const std::string& id)
{
	if (id.find(":")!=id.npos)
	{
		throw InvalidArgumentException("Old style node-ids are no longer supported as of Robot Raconteur version 0.5.  Please use UUID node-id format");
	}
	else
	{
		...
		if (id2.size() != 32) throw InvalidArgumentException("Invalid NodeID");
...
NodeID::NodeID(boost::uuids::uuid id)
{
	this->id=id;
}
...
const boost::uuids::uuid NodeID::ToUuid() const
{
	return id;
}
...
bool NodeID::operator==(const NodeID &id2) const
{
...
bool NodeID::IsAnyNode() const
{
	return id.is_nil();
}

NodeID NodeID::GetAny()
{
	return NodeID();
}
